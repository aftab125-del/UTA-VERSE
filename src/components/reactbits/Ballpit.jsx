...
class x {
  ...
  dispose() {
    ...
    this.clear();
    this.#t?.dispose();
    // Keep the canvas context reusable across React development remounts.
    // renderer.dispose() releases Three resources; forceContextLoss() would
    // poison the same canvas before Strict Mode's second effect setup.
...
const BALLPIT_COLORS = Array.from({ length: 200 }, (_, index) => {
  const distributedSlot = (index * 7) % 20;

  if (distributedSlot < 6) {
    return distributedSlot % 2 === 0 ? 0x2924a8 : 0x3730c9;
  }

  if (distributedSlot < 10) {
    return distributedSlot % 2 === 0 ? 0x6638d9 : 0x7545e8;
  }

  if (distributedSlot < 13) {
    return 0x29292b;
  }

  if (distributedSlot < 17) {
    return 0xafafaf;
  }
